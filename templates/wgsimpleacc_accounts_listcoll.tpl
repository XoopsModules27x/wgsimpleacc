        ...
        <div href="#item-<{$listitem.id}>" class="list-group-item" data-bs-toggle="collapse">
            ...
            <span class="pull-right">
                <{if $listitem.income|default:false}>
                    <img class="wgsa-img" src="<{$wgsimpleacc_icon_url}>/32/incomes.png" title="<{$smarty.const._MD_WGSIMPLEACC_CLASS_INCOME}>" alt="<{$smarty.const._MD_WGSIMPLEACC_CLASS_INCOME}>">
                <{/if}>
                <{if $listitem.expenses|default:false}>
                    <img class="wgsa-img" src="<{$wgsimpleacc_icon_url}>/32/expenses.png" title="<{$smarty.const._MD_WGSIMPLEACC_CLASS_EXPENSES}>" alt="<{$smarty.const._MD_WGSIMPLEACC_CLASS_EXPENSES}>">
                <{/if}>
                ...
                <button class="btn btn btn-outline-primary wgsa-btn-list<{if $listitem.tracount == 0}> disabled<{/if}>"
                        onclick="location.href='transactions.php?op=list&displayfilter=1&amp;acc_id=<{$listitem.id}>&amp;dateFrom=<{$dateFrom}>&amp;dateTo=<{$dateTo}>'" target="_blank" type="button">( <{$listitem.tracount}> ) <{$smarty.const._MD_WGSIMPLEACC_TRANSACTIONS}></button>
